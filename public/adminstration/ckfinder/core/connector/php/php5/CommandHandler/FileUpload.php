...
class CKFinder_Connector_CommandHandler_FileUpload extends CKFinder_Connector_CommandHandler_CommandHandlerBase
{
    ...
    protected $command = "FileUpload";
	 public function autoRotateImage($image, $size=800,$quantity=85) {
	  $imagick_type = new Imagick();
	  $imagick_type->readImage($image);
	  $compression_type = Imagick::COMPRESSION_JPEG;
	  $imageprops = $imagick_type->getImageGeometry();
    ...
    {   $bb = array();
        ...
        if($uploadedFile['type']=='image/jpg' ||$uploadedFile['type']=='image/png' || $uploadedFile['type']=='image/gif' || $uploadedFile['type']=='image/jpeg')
        { $this->autoRotateImage($sFilePath);}
		return $sFilePath;
    }
}
